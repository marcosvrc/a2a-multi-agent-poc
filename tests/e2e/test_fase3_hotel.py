"""E2E test for Fase 3 (PROJECT_SPEC.md §43): the real Hotel Agent
(TypeScript/LangGraph) answers over A2A and the Planner returns a valid,
schema-compliant HotelResult inside the TravelResponse — proving A2A
interoperability between a Python planner and a Node.js specialist.
Requires the stack running and PLANNER_URL set — same convention as
tests/e2e/test_fase2_flight.py.

    PLANNER_URL=http://localhost:8001 pytest tests/e2e/test_fase3_hotel.py
"""
...
def test_foundation_check_includes_hotel_agent():
    resp = httpx.get(f"{PLANNER_URL}/v1/foundation-check", timeout=30)
    resp.raise_for_status()
    body = resp.json()
    assert "hotel-agent" in body["discovered_agents"]
    assert body["delegation_results"]["hotel-agent"]["status"]["state"] == "completed"


def test_travel_request_returns_real_hotel_options():
    payload = {
        "origin": "Sao Paulo",
    ...
    assert 1 <= len(body["hotel"]["options"]) <= 5
    # Overall status depends on which other specialists are registered in
    # the running stack; see test_fase5_budget.py for the full COMPLETED case.
